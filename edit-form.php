...
$sql = "SELECT title, description, image, user_id FROM tasks WHERE id=:id";
  ...
  <body>
    <div class="form-wrapper text-center">
      <form class="form-signin" action="update-form.php?id=<?= $task['id'];?>" method="POST" enctype="multipart/form-data">
        ...
        <input type="text" id="inputEmail" class="form-control" placeholder="Название" required value="<?= $task['title'];?>">
        <label for="inputEmail" class="sr-only">Описание</label>
        <textarea name="description" class="form-control" cols="30" rows="10" placeholder="Описание"><?= $task['description']?></textarea>
        <input type="file"><img src="<?php
          if(empty($task['image'])){
